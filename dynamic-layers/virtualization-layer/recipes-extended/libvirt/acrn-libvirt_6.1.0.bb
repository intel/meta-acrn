...
RDEPENDS_${PN}-libvirtd += "bridge-utils iptables pm-utils dnsmasq netcat-openbsd"
...
# acrn-libvirt is supported by linux-intel 5.4.46 and ACRN 2.0 onward, so
# Skip processing of this recipe if linux-intel-acrn-sos is not explicitly specified as the
# PREFERRED_PROVIDER for virtual/kernel, PREFERRED_VERSION to 5.4 and PREFERRED_PROVIDER for acrn-hypervisor
# to acrn-hypervisor-dev recipes, which is pointing to 2.0 and higher.
# This avoids runtime libvirt-acrn failures
python () {
    if not (d.getVar("PREFERRED_PROVIDER_virtual/kernel") == "linux-intel-acrn-sos" and \
        d.getVar("PREFERRED_PROVIDER_acrn-hypervisor") == "acrn-hypervisor-dev") :
            bb.warn("!! ACRN libvirt is not supported by ACRN 1.6.1 and 4.19 kernel, You might face runtime failures !!")
}
...
RPROVIDES_${PN} += " libvirt"

PROVIDES_${PN}-libvirtd = "libvirt-libvirtd"
RPROVIDES_${PN}-libvirtd += " libvirt-libvirtd"

PROVIDES_${PN}-virsh = "libvirt-virsh"
RPROVIDES_${PN}-virsh += " libvirt-virsh"
